...
meta = [
      ...
      "arguments" : [
        {
          ...
          "description" : "One or more h5mu files, each carrying the per-modality boolean keep-columns in .obs.\nThe per-sample counts of every input are combined into a single report.\n",
          "example" : [
            "input.h5mu"
          ],
          "must_exist" : true,
        ...
        },
        {
          "type" : "string",
          "name" : "--modality",
          "description" : "The RNA modality to read the RNA and scrublet keep-columns from.",
          "default" : [
            "rna"
          ],
          "required" : false,
        ...
        },
        {
          "type" : "string",
          "name" : "--prot_modality",
          "description" : "The protein modality to read the protein keep-columns from. If not provided, no\nprotein counts are reported.\n",
          "required" : false,
          "direction" : "input",
          "multiple" : false,
          "multiple_sep" : ";"
        },
        {
          ...
          "description" : "The .obs column to group the counts by. If the column is not present in .obs, all\nobservations are treated as a single sample.\n",
          "default" : [
            "sample_id"
          ],
          "required" : false,
        ...
        },
        {
          "type" : "string",
          "name" : "--sample_id",
          "description" : "Fallback sample label per input file, in the same order as --input. Used as the\nsample id when --sample_id_column is absent from a file's .obs (e.g. the workflow\nevent id). When the column is present, it takes precedence and this is ignored.\n",
          "required" : false,
          "direction" : "input",
          "multiple" : true,
          "multiple_sep" : ";"
        ...
        {
          "type" : "string",
          "name" : "--rna_filter_columns",
          "description" : "Boolean .obs columns (in the RNA modality) that together (logical AND) define the\ncells passing the RNA QC filters.\n",
          "required" : false,
          "direction" : "input",
          "multiple" : true,
          "multiple_sep" : ";"
        ...
        {
          "type" : "string",
          "name" : "--scrublet_filter_column",
          "description" : "Boolean .obs column (in the RNA modality) with the scrublet keep-mask. If not\nprovided, the scrublet count column is omitted from the report.\n",
          "required" : false,
          "direction" : "input",
          "multiple" : false,
          "multiple_sep" : ";"
        ...
        {
          "type" : "string",
          "name" : "--prot_filter_columns",
          "description" : "Boolean .obs columns (in the protein modality) that together (logical AND) define\nthe cells passing the protein QC filters.\n",
          "required" : false,
          "direction" : "input",
          "multiple" : true,
          "multiple_sep" : ";"
      ...
      "arguments" : [
        {
          "type" : "string",
          "name" : "--rna_qc_metrics",
          "description" : "Per-cell QC metric .obs columns (in the RNA modality) to aggregate over the cells\nsurviving every filter. One column per (aggregation, metric) is added to the report,\nnamed `<modality>_<aggregation>_<metric>_after_all_filter`.\n",
          "default" : [
            "total_counts",
            "pct_counts_mt",
            "n_genes_by_counts"
        ...
        {
          "type" : "string",
          "name" : "--prot_qc_metrics",
          "description" : "Per-cell QC metric .obs columns (in the protein modality) to aggregate over the cells\nsurviving every filter. Only used when --prot_modality is provided.\n",
          "default" : [
            "total_counts",
            "n_genes_by_counts"
          ],
        ...
        {
          "type" : "string",
          "name" : "--qc_metric_aggregations",
          "description" : "Aggregation functions applied to each QC metric over the surviving cells, per sample.\n",
          "default" : [
            "median",
            "mean",
            "max"
      ...
      "arguments" : [
        {
          ...
          "description" : "Output TSV file with the per-sample cell counts.",
          "example" : [
            "cell_counts.tsv"
          ],
          "must_exist" : true,
  ...
  "description" : "Report the number of cells (observations) that pass each QC filter stage, per sample.\nReads boolean keep-columns (True = keep) that were written to .obs by upstream filter\ncomponents (run in flag-only mode, without subsetting) and writes a TSV with one row per\nsample. For each sample the number of cells passing the RNA filters, the scrublet filter,\nthe protein filters, and the combined (intersection of all) filters is reported.\n",
  "test_resources" : [
    {
      "type" : "python_script",
      "path" : "test.py",
  ...
  "build_info" : {
    "config" : "/home/runner/work/openpipeline_incubator/openpipeline_incubator/src/components/report/cell_count_report/config.vsh.yaml",
    "runner" : "nextflow",
    "engine" : "docker",
...
par = {
  ...
  'sample_id': $( if [ ! -z ${VIASH_PAR_SAMPLE_ID+x} ]; then echo "r'${VIASH_PAR_SAMPLE_ID//\\'/\\'\\"\\'\\"r\\'}'.split(';')"; else echo None; fi ),
  'rna_filter_columns': $( if [ ! -z ${VIASH_PAR_RNA_FILTER_COLUMNS+x} ]; then echo "r'${VIASH_PAR_RNA_FILTER_COLUMNS//\\'/\\'\\"\\'\\"r\\'}'.split(';')"; else echo None; fi ),
  'scrublet_filter_column': $( if [ ! -z ${VIASH_PAR_SCRUBLET_FILTER_COLUMN+x} ]; then echo "r'${VIASH_PAR_SCRUBLET_FILTER_COLUMN//\\'/\\'\\"\\'\\"r\\'}'"; else echo None; fi ),
  'prot_filter_columns': $( if [ ! -z ${VIASH_PAR_PROT_FILTER_COLUMNS+x} ]; then echo "r'${VIASH_PAR_PROT_FILTER_COLUMNS//\\'/\\'\\"\\'\\"r\\'}'.split(';')"; else echo None; fi ),
  'rna_qc_metrics': $( if [ ! -z ${VIASH_PAR_RNA_QC_METRICS+x} ]; then echo "r'${VIASH_PAR_RNA_QC_METRICS//\\'/\\'\\"\\'\\"r\\'}'.split(';')"; else echo None; fi ),
  'prot_qc_metrics': $( if [ ! -z ${VIASH_PAR_PROT_QC_METRICS+x} ]; then echo "r'${VIASH_PAR_PROT_QC_METRICS//\\'/\\'\\"\\'\\"r\\'}'.split(';')"; else echo None; fi ),
  'qc_metric_aggregations': $( if [ ! -z ${VIASH_PAR_QC_METRIC_AGGREGATIONS+x} ]; then echo "r'${VIASH_PAR_QC_METRIC_AGGREGATIONS//\\'/\\'\\"\\'\\"r\\'}'.split(';')"; else echo None; fi ),
  'output': $( if [ ! -z ${VIASH_PAR_OUTPUT+x} ]; then echo "r'${VIASH_PAR_OUTPUT//\\'/\\'\\"\\'\\"r\\'}'"; else echo None; fi ),
  'output_column_rna': $( if [ ! -z ${VIASH_PAR_OUTPUT_COLUMN_RNA+x} ]; then echo "r'${VIASH_PAR_OUTPUT_COLUMN_RNA//\\'/\\'\\"\\'\\"r\\'}'"; else echo None; fi ),
  'output_column_scrublet': $( if [ ! -z ${VIASH_PAR_OUTPUT_COLUMN_SCRUBLET+x} ]; then echo "r'${VIASH_PAR_OUTPUT_COLUMN_SCRUBLET//\\'/\\'\\"\\'\\"r\\'}'"; else echo None; fi ),
  'output_column_prot': $( if [ ! -z ${VIASH_PAR_OUTPUT_COLUMN_PROT+x} ]; then echo "r'${VIASH_PAR_OUTPUT_COLUMN_PROT//\\'/\\'\\"\\'\\"r\\'}'"; else echo None; fi ),
  'output_column_all': $( if [ ! -z ${VIASH_PAR_OUTPUT_COLUMN_ALL+x} ]; then echo "r'${VIASH_PAR_OUTPUT_COLUMN_ALL//\\'/\\'\\"\\'\\"r\\'}'"; else echo None; fi )
}
meta = {
...
meta["defaults"] = [
  ...
  "container" : {
    "registry" : "ghcr.io",
    "image" : "viash-hub/custom-singlecell/components/report/cell_count_report",
    "tag" : "build_main"
  },
  "label" : [
    "singlecpu",
