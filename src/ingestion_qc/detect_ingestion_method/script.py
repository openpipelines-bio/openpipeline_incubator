...
def main(par):
    ...
    if mod_uns.get(par["output_uns_ingestion_method"]) != detected_method:
        raise ValueError(f"Field .uns['{par['output_uns_ingestion_method']}'] already exists and contains different value (`{mod_uns[par['output_uns_ingestion_method']]}`) than detected method (`{detected_method}`).")

    # copy input to output
    shutil.copy(par["input"], par["output"])

    # add method to uns if need be
